...
## Path to AudioStreamPlayer3D node for explosion sound
@export var audio_player_path: NodePath = "AudioStreamPlayer3D"

## Particle effect to spawn on shatter
@export var shatter_particles: PackedScene

## Offset for particle spawn position
@export var particle_offset: Vector3 = Vector3.ZERO
...
func initialize(pos, rot) -> void: 
	...
	if shatter_particles:
		...
		particles.global_position = global_position + particle_offset
		
		# Auto-remove particles after some time if they're GPUParticles3D
		if particles is GPUParticles3D:
			particles.emitting = true
			var particle_lifetime = particles.lifetime + particles.lifetime * particles.lifetime_randomness
			await get_tree().create_timer(particle_lifetime + 2.0).timeout
			particles.queue_free()
		
		DebugLogger.debug(module_name, "Spawned shatter particles")
