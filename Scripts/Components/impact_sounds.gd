...
func _ready():
	...
	DebugLogger.register_module("ImpactSound")
	
	# Get parent RigidBody3D
	parent_body = get_parent() as RigidBody3D
	if not parent_body:
		DebugLogger.error("ImpactSound", "ImpactSound must be a child of a RigidBody3D")
		queue_free()
		return
...
func _on_body_entered(body: Node):
	...
	# Play impact sound at current position
	_play_impact_sound(velocity, parent_body.global_position)
	last_impact_time = current_time

func _play_impact_sound(velocity: float, impact_position: Vector3):
	if impact_sounds.is_empty():
		DebugLogger.warning("ImpactSound", "No impact sounds configured")
		return
	...
	var final_pitch = pitch_base + pitch_random
	
	# Use Audio singleton to play sound at impact position
	var audio_player = Audio.play_sound_3d(sound, true, final_pitch, volume, audio_bus)
	audio_player.global_position = impact_position
	
	DebugLogger.debug("ImpactSound", "Playing impact: velocity=%.2f, volume=%.2f dB, pitch=%.2f, pos=%s" % [velocity, volume, final_pitch, impact_position])
