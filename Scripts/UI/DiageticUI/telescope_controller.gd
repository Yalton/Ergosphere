...
func _input(event: InputEvent) -> void:
	# Handle mouse wheel scrolling directly on the RichTextLabel
	if event is InputEventMouseButton:
		if output_area and event.pressed:
			if event.button_index == MOUSE_BUTTON_WHEEL_UP or event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
				# Focus output area for scrolling
				if not output_area.has_focus():
					output_area.grab_focus()
					DebugLogger.debug(module_name, "Auto-focused output area for scrolling")
				
				# Let the RichTextLabel handle its own scrolling naturally
				# Just mark as handled so it doesn't bubble up
