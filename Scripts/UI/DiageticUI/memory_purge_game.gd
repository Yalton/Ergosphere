...
@export var initial_block_corruption_interval: float = 2.0
@export var min_block_corruption_interval: float = 0.5
@export var block_corruption_lifetime: float = 5.0
...
@export var block_corrupted_color: Color = Color(1.0, 0.3, 0.3)
...
var block_states: Dictionary = {} # block -> {"corrupted": bool, "block_corruption_timer": float}
var blocks_purged: int = 0
var block_corruption_timer: Timer
var current_block_corruption_interval: float
...
func _ready() -> void:
	...
	# Create block corruption timer
	block_corruption_timer = Timer.new()
	block_corruption_timer.timeout.connect(_corrupt_random_block)
	add_child(block_corruption_timer)
...
func _on_start_pressed() -> void:
	...
	# Reset game state
	blocks_purged = 0
	current_block_corruption_interval = initial_block_corruption_interval
	_update_counter()
	_update_progress()
	...
	for block in memory_blocks:
		...
		block_states[block]["block_corruption_timer"] = 0.0
	
	# Start game
	is_game_active = true
	block_corruption_timer.start(current_block_corruption_interval)
...
func _corrupt_random_block() -> void:
	...
	current_block_corruption_interval = max(min_block_corruption_interval, current_block_corruption_interval * 0.98)
	block_corruption_timer.wait_time = current_block_corruption_interval
...
func _corrupt_block(block: Button) -> void:
	...
	block_states[block]["block_corruption_timer"] = block_corruption_lifetime
	
	# Animate corruption
	var tween = create_tween()
	tween.tween_property(block, "modulate", block_corrupted_color, 0.2)
...
func _process(delta: float) -> void:
	...
	# Update block corruption timers
	for block in memory_blocks:
		if block_states[block]["corrupted"]:
			block_states[block]["block_corruption_timer"] -= delta
			
			# Auto-cleanse if block corruption timer expires
			if block_states[block]["block_corruption_timer"] <= 0:
